...
`;
  ...
  var initColorShiftLoop = (schemes) => {
    let i = 0;
  ...
  var generateColorPalette = async (mainColor, numColors) => {
    ...
    const palette = await fetch(`https://www.thecolorapi.com/scheme?hex=${mainColor}&mode=${modeStr}&count=${numColors}`).then(
      (response) => response.json()
    );
    const colorArray = palette.colors.map((color) => color.hex.value.substring(1));
    return colorArray;
  };
  async function waitForAlbumArt() {
  ...
  var initAlbumArtBasedColor = (scheme) => {
    Spicetify.Player.addEventListener("songchange", async () => {
      ...
      if (albumArtSrc) {
        ...
        const mainColor = await getColorFromImage(albumArtSrc);
        ...
        const orderedColorMap = new Map(
          [...colorMap.entries()].sort((a, b) => {
            const aColor = chroma_js_default(a[0]);
            const bColor = chroma_js_default(b[0]);
  ...
  var getParamsFromGithubRaw = (url) => {
  ...
  var addExtensionToSpicetifyConfig = (main) => {
    if (!main)
      return;
...
`;
  ...
  (async function init2() {
    ...
    const initializeExtension = (extensionKey) => {
      ...
      if (!extensionManifest || !extensionManifest.extensionURL)
        return;
      console.debug("Initializing extension: ", extensionManifest);
      const script2 = document.createElement("script");
      script2.defer = true;
      script2.src = extensionManifest.extensionURL;
      if (isGithubRawUrl(script2.src)) {
        const { user, repo, branch, filePath } = getParamsFromGithubRaw(extensionManifest.extensionURL);
        if (!user || !repo || !branch || !filePath)
          return;
        script2.src = `https://cdn.jsdelivr.${tld}/gh/${user}/${repo}@${branch}/${filePath}`;
        if (filePath.endsWith(".mjs"))
          script2.type = "module";
      }
      script2.src = `${script2.src}?time=${Date.now()}`;
    ...
    const initializeTheme = async (themeKey) => {
      ...
      if (existingMarketplaceThemeCSS)
        existingMarketplaceThemeCSS.remove();
      ...
      if (themeManifest.include?.length) {
        for (const script2 of themeManifest.include) {
          const newScript = document.createElement("script");
          let src = script2;
          if (isGithubRawUrl(script2)) {
            const { user, repo, branch, filePath } = getParamsFromGithubRaw(script2);
            if (!user || !repo || !branch || !filePath)
              return;
          ...
          addExtensionToSpicetifyConfig(script2);
        }
      }
    };
    ...
    if (!tld) {
      if (window.navigator.onLine) {
        ...
        Spicetify.showNotification(t("notifications.noCdnConnection"), true, 5e3);
      } else {
        window.addEventListener("online", init2, { once: true });
      }
      return;
    }
    ...
    if (installedTheme) {
      if (localTheme.toLocaleLowerCase() !== "marketplace") {
        Spicetify.showNotification(t("notifications.wrongLocalTheme"), true, 5e3);
        return;
      }
  ...
  async function queryRepos(type, pageNum = 1) {
    const BLACKLIST = JSON.parse(window.sessionStorage.getItem("marketplace:blacklist") || "[]");
    let url = `https://api.github.com/search/repositories?per_page=${ITEMS_PER_REQUEST}&q=${encodeURIComponent(`topic:spicetify-${type}s`)}`;
    if (pageNum)
      url += `&page=${pageNum}`;
    const allRepos = JSON.parse(window.sessionStorage.getItem(`spicetify-${type}s-page-${pageNum}`) || "null") || await fetch(url).then((res) => res.json()).catch(() => null);
    if (!allRepos?.items) {
  ...
  async function loadPageRecursive(type, pageNum) {
    ...
    if (remainingResults > 0)
      return await loadPageRecursive(type, pageNum + 1);
